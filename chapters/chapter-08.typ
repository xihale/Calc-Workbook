...
#question(
  ...
  a: [设四边形为 $A B C D$，对角线交于 $M$ 且互相平分，则 $arrow(O A) + arrow(O C) = 2 arrow(O M), arrow(O B) + arrow(O D) = 2 arrow(O M)$，故 $arrow(O A) + arrow(O C) = arrow(O B) + arrow(O D)$。
    移项得 $arrow(A B) = arrow(D C)$ 且 $arrow(A D) = arrow(B C)$，从而两组对边分别平行且相等，所以 $A B C D$ 为平行四边形。],
)
...
#question(
  ...
  a: [×。（由 $bold(a) times (bold(b) - bold(c)) = 0$ 只能得出 $bold(b) - bold(c)$ 与 $bold(a)$ 平行）],
)
...
#question(
  ...
  a: [(1) $3, 5bold(i) + bold(j) + 7bold(k)$；(2) $-18, 10bold(i) + 2bold(j) + 14bold(k)$；(3) $3 / (2 sqrt(21))$。],
)
...
#question(
  ...
  a: [$plus.minus (3/sqrt(17), -2/sqrt(17), -2/sqrt(17))$],
)
