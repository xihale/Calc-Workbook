...
#question(
  [
    8. 函数 $f(x, y) = (x^2 - y^2)^(-1) cos (y/x)$ 的间断点为
  ],
...
#question(
  ...
  a: [$2x y e^(y(x^2 + y^2)) dif x + (x^2 + 3 y^2) e^(y(x^2 + y^2)) dif y$。],
)
...
#question(
  [
    3. 设函数 $y = y(x)$ 由方程 $1 + x^2 y = e^y$ 所确定，则 $dif y / dif x =$
  ],
  a: [$(2 x y)/(e^y - x^2)$],
)
...
#question(
  [
    5. 设函数 $f(x, y, z) = x^2 + 2 y^2 + 3 z^3 + x y + 3 x - 2 y - 6 z$ ，则 $bold(grad) f(0,0,0) =$
  ],
  a: [$(3, -2, -6)$ .],
)
