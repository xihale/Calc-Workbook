...
#question(
  ...
  a: [$1/2 iint_D (x^2 + y^2) dif sigma$。],
)
...
#question(
  ...
  a: [
    ...
    $1 <= r <= 2, 0 <= theta <= pi/4$。
    被积函数
    $y/x = tan theta$，面积元 $dif sigma = r dif r dif theta$。

    因此
    $iint_D y/x dif sigma
    = int_0^(pi/4) int_1^2 tan theta dot r dif r dif theta$
    $= (int_1^2 r dif r)(int_0^(pi/4) tan theta dif theta)$
    $= (3/2) dot [-ln(cos theta)]_0^(pi/4)$
    $= (3/2) dot (1/2 ln 2)
    = 3/4 ln 2$。
  ],
)
...
#question(
  [
    3. 设 $Omega$ 是由曲面 $z = x^2 + y^2$ 和 $z = 2 - x^2 - y^2$ 所围成的空间闭区域，则 $iiint_Omega f(x, y, z) dif v$ 在柱面坐标系下的三次积分为
  ],
  a: [$int_0^(2 pi) dif theta int_0^1 dif rho int_(rho^2)^(2 - rho^2) f(rho cos theta, rho sin theta, z) rho dif z$。],
)
