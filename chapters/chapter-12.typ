...
A. $sum_(n=0)^oo x^(2n) / n!$ 

B. $sum_(n=0)^oo (-1)^n x^(2n) / n!$ 

C. $sum_(n=0)^oo x^n / n!$ 

D. $sum_(n=0)^oo (-1)^n x^n / n!$
], a: [B.])
...
D. $ S(x) = cases(f(x) & if x != (2k-1) pi, pi/2 & if x = (2k-1) pi) $（$k$ 为整数）
], a: [D.])
...
A. $a_n = 0$ ($n = 0, 1, 2, dots$)，$b_n != 0$ ($n = 1, 2, dots$)

B. $b_n = 0$ ($n = 1, 2, dots$)，$a_(2k+1) = 0$ ($k = 0, 1, 2, dots$)

C. $a_n != 0$ ($n = 0, 1, 2, dots$)，$b_n = 0$ ($n = 1, 2, dots$)
...
A. 在区间 $[-3, 3)$ 外进行周期延拓，级数在区间 $(-3, 0) cup (0, 3)$ 内收敛于 $f(x)$ 

B. 进行奇延拓，级数在区间 $(-3, 0) cup (0, 3)$ 内收敛于 $f(x)$ 

C. 进行偶延拓，级数在区间 $[-3, 3]$ 上收敛于 $f(x)$ 

D. 在区间 $[-3, 3)$ 外进行周期延拓，级数在区间 $(-3, 3)$ 上收敛于 $f(x)$，且在端点处收敛于均值。
], a: [D.])
...
C. $y/4$ （注：原题系数通常为 $x y'' + y' - y = 0$ 变形，需核对）
...
#question([
  2. 设 $f(x)$ 为连续函数，$F(t) = integral_1^t dif y integral_y^t f(x) dif x$，则 $F'(2) = $ #ans[$integral_1^2 f(x) dif x$]。
], a: [$integral_1^2 f(x) dif x$])
...
C. $x^2/3 + y^2/3 - 4z^2 = 1$ 

D. $x^2/3 - 4y^2 + z^2/3 = 1$
], a: [D.])

#question([
  4. 设 $L: x^2 + y^2 = 1$，取顺时针方向，则 $integral.cont_L (x dif y - y dif x) / (x^2 + y^2) = $ #ans[$-2 pi$]。
], a: [$-2 pi$])
...
#question([
  9. 设曲面 $Sigma$ 为上半球面 $z = sqrt(2 - x^2 - y^2)$，则 $integral.double_Sigma (y + 1) dif S = $ #ans[$4 pi$]。
], a: [$4 pi$])
...
], a: [驻点为 $(2, -2)$，$A = -2, B = 0, C = -2$，$AC-B^2 = 4 > 0$，且 $A < 0$，该点为极大值点。])

#question([
  12. 计算 $integral.double_D cos(x/y) dif x dif y$，其中 $D$ 为由直线 $x = 0, y = pi/2, y = pi$ 及抛物线 $x = y^2$ 所围成的平面区域。
], a: [原式 $= integral_{pi/2}^pi dif y integral_0^{y^2} cos(x/y) dif x = integral_{pi/2}^pi y sin y dif y = [sin y - y cos y]_{pi/2}^pi = pi - 1$。])
...
#question([
  19. 计算 $ I = integral.double_Sigma [f_y(x, y) + x^3] dif y dif z + [y^3 - f_x(x, y)] dif z dif x + z^3 dif x dif y $，其中 $f(x, y)$ 具有二阶连续偏导数，$Sigma$ 为锥面 $z = sqrt(x^2 + y^2)$ ($0 <= z <= 1$) 的下侧。
], a: [$-3 pi / 10$])
...
#question([
  2. 函数 $z = 2x^3 - 4x^2 + 2xy - y^2$ 的极大值点为（ #choice[A] ）。
A. $(0, 0)$ 
...
A. $integral.double_Sigma x dif S = 4 integral.double_{Sigma_1} x dif S$ 

B. $integral.double_Sigma y dif S = 4 integral.double_{Sigma_1} y dif S$ 

C. $integral.double_Sigma z dif S = 4 integral.double_{Sigma_1} x dif S$ 

D. $integral.double_Sigma x y z dif S = 4 integral.double_{Sigma_1} x y z dif S$
], a: [A.])
...
#question([
  10. 设 $I = integral.triple_Omega dif v / (1 + x^2 + y^2)$，其中空间闭区域 $Omega$ 由 $z = sqrt(x^2 + y^2)$ 与 $z = 1$ 围成，则 $I$ 在柱面坐标系下的三次积分为 #ans[$integral_0^{2 pi} dif theta integral_0^1 rho dif rho integral_rho^1 rho / (1 + rho^2) dif z$]。
], a: [$integral_0^{2 pi} dif theta integral_0^1 rho dif rho integral_rho^1 rho / (1 + rho^2) dif z$])

#question([
  11. 设 $f(x)$ 为连续函数，$F(t) = integral_0^t dif y integral_y^t f(x) dif x$ ($t > 0$)，将 $F(t)$ 交换积分次序后有 #ans[$integral_0^t dif x integral_0^x f(x) dif y = integral_0^t x f(x) dif x$]。
], a: [$integral_0^t x f(x) dif x$])

#question([
  12. 设 $L$ 是直线 $3x + 4y = 12$ ($0 <= x <= 4$) 的一段，则 $integral_L (3x + 4y) dif s = $ #ans[$60$]。
], a: [60])
...
#question([
  17. 计算 $integral_L (3 sin x + 2 x y - 4 x) dif x + (x^2 - e^y) dif y$，其中 $L$ 为摆线 $x = t - sin t, y = 1 - cos t$ 上从点 $O(0, 0)$ 到点 $A(pi, 2)$ 的一段弧。
], a: [$4 - e^2 - pi$])
...
#question([
  19. 计算 $ I = integral.double_Sigma x z^2 dif y dif z + (x^2 y - z^3) dif z dif x + (2 x y + y^2 z) dif x dif y $，其中 $Sigma$ 是球面 $x^2 + y^2 + z^2 = a^2$ ($a > 0$) 上半部分的上侧。
], a: [$2/5 pi a^5$])
...
#question([
  7. 设曲面 $Sigma : x^2 + y^2 + z^2 = R^2 (R > 0)$ 的外法向量的方向余弦为 $cos alpha , cos beta , cos gamma$ ，则 $integral.double_Sigma (x cos alpha + y cos beta + z cos gamma ) dif S = $ #ans[$4 pi R^3$]。
A. $pi R^3$ 
...
#question([
  9. 设空间闭区域 $Omega = ((x, y, z) | x^2 + y^2 + z^2 <= 1)$ ，则 $integral.triple_Omega sqrt (x^2 + y^2 + z^2) dif v = $ #ans[$pi/2$]。
A. $integral.triple_Omega dif x \, dif y \, dif z$ 
...
#question([
  13. $integral_0^2 dif x integral_x^2 e^(-y^2) dif y = $ #ans[$1/2 (1 - e^-4)$]。
], a: [$1/2 (1 - e^-4)$])
...
#question([
  15. 设函数 $f(x) = |x - 1/2|, b_n = 2 integral_0^1 f(x) sin(n pi x) dif x$ ，其正弦级数为 $S(x) = sum_(n=1)^( oo ) b_n sin(n pi x)$ ，则 $S(-2021) = #ans[$0$]$.
], a: [$S(x)$ 是 $f(x)$ 的正弦级数，根据狄利克雷收敛定理，级数在 $x = k pi / pi = k$ (整数) 处的和为 0 (因为所有项 $sin(n pi k) = 0$)。故 $S(-2021) = 0$。])

#question([
  16. 设 $L: |x| + |y| = 1$ ，则 $integral.cont_L (dif s)/(|x| + |y|) = #ans[$4 sqrt(2)$]$.
], a: [在曲线 $L$ 上，$|x| + |y| = 1$ 恒成立。
  则 $integral.cont_L (dif s)/(|x| + |y|) = integral.cont_L 1 dif s$，即为正方形 $L$ 的周长。
...
#question([
  20. 计算 $integral_L (x e^y - 2y) dif y + (e^y + x - y) dif x$，其中 $L$ 为圆 $x^2 + y^2 = ax$ ($a > 0$) 的上半部分，取逆时针方向。
], a: [$a^2 - pi a^2/8$])

#question([
  21. 计算 $integral.double_Sigma x dif y dif z - y dif z dif x + (z^2 + x) dif x dif y $，其中 $Sigma$ 为半球面 $z = sqrt (4 - x^2 - y^2)$ 的下侧。
...
#question([
  22. 设幂级数 $sum#ans[$(1) 系数 $a_n = 1/(n dot 3^n)$。
  ...
  故 $S(x) = -ln(1 - (x-3)/3) = ln(3/(6-x))$， $x in [0, 6)$。$](n=1)^( oo ) (x-3)^n / (n dot 3^n)$ , 求该幂级数的：（1）收敛半径；（2）收敛域；（3）和函数.
...
#question([
  6. 已知 $|a| = 4$ ，向量 $bold(a)$ 与轴 $u$ 的夹角是 $(pi /3)$ ，则 $op (P r j)#ans[$2.$]u bold(a) =  #ans[$2.$] .$
], a: [2.])
...
即
$$
(2 + lambda) x - 3 y + 3 lambda z + 5 + lambda = 0.
$$
将点(2,1,3)代入上述方程，解得 $lambda = -(1/2)$ ，则过点 $M$ 与已知直线的平面方程为
$$
3 x - 6 y + 3 z + 9 = 0, \text{即} x - 2 y + z + 3 = 0.
$$
过点 $M$ 且垂直于已知直线的平面方程为
$$
3 x + 2 y - z - 5 = 0,
...
$$
$$
\begin{array}{l} \frac {partial^ {2} z}{partial x partial y} = (partial)/(partial y) ((partial z)/(partial x)) = (partial)/(partial y) (y f \_ {1} ^ {\prime} + \frac {f \_ {2} ^ {\prime}}{y} + 2 x g ^ {\prime})   = f \_ {1} ^ {\prime} + y (f \_ {1 1} ^ {\prime \prime} x + f \_ {1 2} ^ {\prime \prime} (-x/y^2)) + (-1/y^2) f \_ {2} ^ {\prime} + (1/y) (f \_ {2 1} ^ {\prime \prime} x + f \_ {2 2} ^ {\prime \prime} (-x/y^2)) + 2x g'' (-2y)   = f \_ {1} ^ {\prime} - \frac {f \_ {2} ^ {\prime}}{y ^ {2}} + x y f \_ {1 1} ^ {\prime \prime} - \frac {x}{y ^ {3}} f \_ {2 2} ^ {\prime \prime} - 4 x y g ^ {\prime \prime}.   \end{array}
$$])
...
因此，和函数为
$$
S(x) = \frac {2}{(1 - (x - 1)) ^ {3}} = \frac {2}{(2 - x) ^ {3}}.
$$])
...
同理，当 $g(0,0) = 0$ 时，
$$
f \_ {x} ^ {\prime} (0, 0) = 0, \quad f \_ {y} ^ {\prime} (0, 0) = 0.
$$
