...
#question(
  ...
  p: [由于 $(lim_(n arrow oo) ((1)/(n^(1+((1))/((n))))))/((((1))/((n))))=(lim_(n arrow oo) 1)/(n^(((1))/((n))))= 1$，由极限比较审敛法可知原级数与调和级数 $(sum 1)/(n)$ 同敛散，故原级数发散。选项 A、B 是错误结论，C 不成立（实际上当 $n$ 较大时 $<(1)/(n)$），故选 D。],
)
...
#question(
  ...
  a: [
    ...
    其中 \
    $f(pi)=pi e^pi, f(-pi)=-pi e^(-pi)$。 \
    故 \
    $S(pi)=(1)/(2[pi e^pi-pi e^(-pi)])=pi sinh pi$。
  ],
)
== 第六节 一般周期函数的傅里叶级数
...
#question(
  [
    ...
    A. 必同时收敛 #quad B. 必同时发散 #quad C. 可能同时发散 #quad D. 不可能同时收敛
  ],
)
...
#question(
  ...
  p: [
    设 $F(x,y,z)=x^2+2y^2+z-x e^z-4$。计算梯度： \
...
#question(
  ...
  a: [
    ...
    因而 \
    $I=int_((pi)/(2))^pi y sin y dif y =[-y cos y+sin y]_((pi)/(2))^pi =pi-1$。
  ],
)
...
#question(
  ...
  a: [
    ...
    $a_(2k+1)=(1)/(k!)$。 \
    故 \
    $S(x)=sum_(k=0)^oo a_(2k+1) x^(2k+1) =(sum_(k=0)^oo x^(2k+1))/(k!)=(x sum_(k=0)^oo (x^2)^k)/(k!)=x e^(x^2)$。
  ],
)
...
#question(
  ...
  a: [
    ...
    交换次序得 \
    $0<=y<=2, 0<=x<=y$，故 \
    $I=int_0^2 dif y int_0^y e^(-y^2) dif x =int_0^2 y e^(-y^2) dif y$。 \
    令 $u=y^2$，得 \
    $I=(1)/(2 int_0^4 e^(-u) dif u)=(1)/(2(1-e^(-4)))$。
  ],
)
...
#question(
  ...
  a: [
    ...
    故 \
    $L=(1)/(2 int_1^e (y+((1))/((y))) dif y)=(1)/(2[((y^2))/((2))+ln y]_1^e)=(1)/(4(e^2+1))$。
  ],
)
...
#question(
  ...
  a: [
    ...
    因而 \
    $I=iint_D e^(-x^2-y^2) dif x dif y =int_0^(2pi) int_0^R e^(-r^2) r dif r dif theta$。 \
    先算径向积分，令 $u=r^2$，$dif u=2r dif r$： \
    $int_0^R e^(-r^2) r dif r =(1)/(2 int_0^(R^2) e^(-u) dif u)=(1)/(2 (1-e^(-R^2)))$。 \
    故 \
    $I=(int_0^(2pi) 1)/(2 (1-e^(-R^2)) dif theta)=pi(1-e^(-R^2))$。
  ],
)
...
#question(
  ...
  a: [
    ...
    $f_(y y)''=2e^(2x)$。 \
    在 $((1)/(2),-1)$ 处有 \
    $A=f_(x x)''=2e>0, B=f_(x y)''=0, C=f_(y y)''=2e$。 \
    判别式 \
