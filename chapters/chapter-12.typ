...
A. 在区间 $[-3, 3)$ 外进行周期延拓，级数在区间 $(-3, 0) union (0, 3)$ 内收敛于 $f(x)$ 

B. 进行奇延拓，级数在区间 $(-3, 0) union (0, 3)$ 内收敛于 $f(x)$ 
...
#question([
  11. 设函数 $z = f(x, y)$ 的全微分为 $d z = (4 - 2x) dif x - (2y + 4) dif y$，试确定 $f(x, y)$ 的极值点，并判别该点是极大值点还是极小值点。
...
], a: [$d z = (1/y f_1' - y/x^2 f_2') dif x + (-x/y^2 f_1' + 1/x f_2') dif y$])
...
A. $iint_Sigma x dif S = 4 iint_(Sigma_1) x dif S$ 

B. $iint_Sigma y dif S = 4 iint_(Sigma_1) y dif S$ 

C. $iint_Sigma z dif S = 4 iint_(Sigma_1) x dif S$ 

D. $iint_Sigma x y z dif S = 4 iint_(Sigma_1) x y z dif S$
], a: [A.])
...
#question([
  8. 设函数 $f$ 可微，$z = f(x + y, x y)$ 在点 $(x, y)$ 处的全微分 $d z = $ #ans[$(f_1' + y f_2') dif x + (f_1' + x f_2') dif y$]。
...
A. $iiint_Omega dif x   dif y   dif z$ 

B. $int_0^(2 pi ) dif theta int_0^( pi ) d int_0^1 rho^3 sin dif rho$ 

C. $int_0^(2 pi ) dif theta int_0^ pi d int_0^1 rho^3 sin theta dif rho$ 

D. $int_0^(2 pi ) dif theta int_0^(2 pi ) d int_0^1 rho^3 sin dif rho$
], a: [B.])
...
#question([
  12. 设函数 $z = f(x, y)$ 由方程 $e^z + x y z + x + cos x = 2$ 所确定，则 $d z|_((0,1)) = $ #ans[$-d x$]。
], a: [$-d x$])
...
#question([
  16. 设 $L: |x| + |y| = 1$ ，则 $oint_L (d s)/(|x| + |y|) = $ #ans[$4 sqrt(2)$].
], a: [在曲线 $L$ 上，$|x| + |y| = 1$ 恒成立。
  则 $oint_L (d s)/(|x| + |y|) = oint_L 1 dif s$，即为正方形 $L$ 的周长。
...
A. $int_0^2 dif y int_(sqrt(8 - y^2))^2 f(x,y) dif x$ 

B. $int_0^2 dif y int_0^(sqrt(8 - y^2))f(x,y) dif x$ 

C. $int_0^2 dif y int_(y^2)^2 f(x,y) dif x$ 

D. $int_0^2 dif y int_y^(sqrt(8 - y^2))f(x,y) dif x$
], a: [D.])
...
3 x - 6 y + 3 z + 9 = 0, text{"即"} x - 2 y + z + 3 = 0.
$ 过点 $M$ 且垂直于已知直线的平面方程为 $
3 x + 2 y - z - 5 = 0,
...
得驻点 $(1/2, - 1)$ .因为 $
f_{x x}'' (1/2, -1) = 2 e = A > 0, quad f_{x y}'' (1/2, -1) = 0 = B, quad f_{y y}'' (1/2, -1) = 2 e = C,
...
sum_ {n = 0} ^ {oo} t ^ {n + 2} = (t ^ {2})/(1 - t), quad | t | \<   1,
$ 对上式两端同时逐项求导两次，得 $
sum_ {n = 0} ^ {oo} (n + 1) (n + 2) t ^ {n} = (2)/((1 - t) ^ {3}).
$ 因此，和函数为 $
S(x) = (2)/((1 - (x - 1)) ^ {3}) = (2)/((2 - x) ^ {3}).
$ ])
...
$ = 3 int_0^(2 pi) dif theta int_0^(pi/4) sin phi dif phi int_0^1 r^4 d r = 3/5 (2 - sqrt(2)) pi . $])
...
原式 $ = int_"OB" P dif x + Q dif y + int_"BA" P dif x + Q dif y $
...
$ f_x' (0, 0) = lim  _ {Delta x arrow 0} (f (Delta x , 0) - f (0 , 0))/(Delta x) = lim  _ {Delta x arrow 0} (| Delta x |)/(Delta x) g (Delta x, 0), $
同理，当 $g(0,0) = 0$ 时，
