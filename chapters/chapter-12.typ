...
A. $iiint_Omega dif x   dif y   dif z$ #quad B. $int_0^(2 pi ) dif theta int_0^( pi ) d int_0^1 rho^3 sin dif rho$ \
C. $int_0^(2 pi ) dif theta int_0^ pi d int_0^1 rho^3 sin theta dif rho$ #quad D. $int_0^(2 pi ) dif theta int_0^(2 pi ) d int_0^1 rho^3 sin dif rho$
], a: [B.])
...
得驻点 $(1/2, - 1)$ .因为 $f_{x x}'' (1/2, -1) = 2 e = A > 0, quad f_{x y}'' (1/2, -1) = 0 = B, quad f_{y y}'' (1/2, -1) = 2 e = C,$ 且 $"AC" - B^2 > 0$ ，所以该驻点为极小值点，其值为 $-(( e)/(2))$])
...
令 $x - 1 = t$ ，记 $S(t) = sum_(n=0)^( oo )(n + 1)(n + 2)t^(n)$ . 由于 $sum_ {n = 0} ^ {oo} t ^ {n + 2} = (t ^ {2})/(1 - t), quad | t | \<   1,$ 对上式两端同时逐项求导两次，得 $sum_ {n = 0} ^ {oo} (n + 1) (n + 2) t ^ {n} = (2)/((1 - t) ^ {3}).$ 因此，和函数为 $S(x) = (2)/((1 - (x - 1)) ^ {3}) = (2)/((2 - x) ^ {3}).$ ])
