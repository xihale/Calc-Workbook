...
#question([
  2. 设函数 $ f(x, y) = cases( x sin ((1)/(y)) + y sin ((1)/(x)), & x y != 0, \\ 0, & x y = 0, ) $ 则 $lim_((x, y) arrow (0, 0)) f(x, y) = ( " " )$ .
A. 不存在
...
#question([
  3. 设函数 $ f(x, y) = cases( ((x y)/( sqrt (x^2 + y^2))), & x^2 + y^2 != 0, \\ 0, & x^2 + y^2 = 0, ) $ 则 $f(x, y)$ ( ).
A. 处处连续
...
#question([
  5. 设函数 $f(x, y) = x^(2) + y^(2) + x y ln ((y)/(x))$ ，则 $f(k x, k y) =$ \_\_\_\_\_\_\_\_.
], a: [$k^2 f(x,y)$])
...
#question([
  7. 设函数 $ f(x, y) = cases( (( tan (x^2 + y^2))/(x^2 + y^2)), & (x, y) != (0, 0), \\ A, & (x, y) = (0, 0), ) $ 要使得 $f(x, y)$ 在点(0,0)处连续，则 $A =$ \_\_\_\_\_\_\_\_.
], a: [1.])
...
#question([
  3. 设函数 $z = y^(x)$ ，则 $ ((( partial z)/( partial x)) +(( partial z)/( partial y))) |_( (x = 2\\ y = 1)) = () $
A. 2 
...
#question([
  5. 设函数 $u = x y + ((y)/(x))$ , 则 $(( partial^2 u)/( partial x^2)) =$ \_\_\_\_\_\_\_\_.
], a: [$((2 y)/(x^3))$])

#question([
  6. 设函数 $u = x ln x y$ ，则 $(( partial^2 u)/( partial x partial y)) =$ \_\_\_\_\_\_\_\_.
], a: [$((1)/(y))$])
...
#question([
  4. 函数 $z = ((y)/(x))$ 当 $x = 2, y = 1, Delta x = 0 .1, Delta y = -0 .2$ 时的全增量 $Delta z =$ 全微分 $(d)z =$
], a: [$-((5)/(42)), - ((1)/(8))$])

#question([
  5. 设函数 $z = (e)^(y(x^2 + y^2))$ ，则 $(d)z =$
], a: [$2 x y (e)^(y(x^2 +y^2)) (d)x + (x^2 +3 y^2) (e)^(y(x^2 +y^2)) (d)y .$])

#question([
  6. 设函数 $z = f(((y)/(x)))$ ，则 $(d)z =$
], a: [$-((y)/(x^2)) f'(((y)/(x))) (d)x + ((1)/(x)) f'(((y)/(x))) (d)y .$])

#prob-type("三、计算题")

#question([
  7. 设函数 $u = a^(x + y z) - ln x^a (a > 0)$ ，求 $(d)u$
], a: [提示: $ (( partial u)/( partial x)) = a^(x + y z) ln a - a x^(-1), (( partial u)/( partial y)) = a^(x + y z) dot z ln a, (( partial u)/( partial z)) = y a^(x + y z) ln a, $
$$
\mathrm {d} u = \left(a ^ {x + y z} \ln a - a x ^ {- 1}\right) \mathrm {d} x + a ^ {x + y z} \ln a (z \mathrm {d} y + y \mathrm {d} z).
$$])
...
$$
\mathrm {d} z = \frac {1}{x ^ {2} + y ^ {2} + \mathrm {e} ^ {x y}} \left\[ (2 x + y \mathrm {e} ^ {x y}) \mathrm {d} x + (2 y + x \mathrm {e} ^ {x y}) \mathrm {d} y \right\].
$$])
...
#question([
  2. 设函数 $z = 3^(x y)$ ，而 $x = f(y)$ 且 $f$ 可导，则 $(( (d)z)/( (d)y)) = ( " " )$ .
...
D. $z_(x)^( )f^( )(y) + z_(y)^( ) - 3^(x y)[x + y f^( )(y)] ln 3$
], a: [B.])

#question([
  3. 设函数 $u = f(x + y, x z)$ ，其中 $f$ 具有二阶连续偏导数，则 $(( partial^2 u)/( partial x partial z)) = ( " " )$
A. $f_(2)^( ) + x f_(11)^( ) + z f_(12)^( ) + x f_(12)^( )$ 

B. $x f_(12)^( ) + x f_(2)^( ) + x z f_(22)^( )$ 

C. $x f_(21)^( ) + x z f_(22)^( )$ 

D. $f_(2)^( ) + x f_(21)^( ) + x z f_(22)^( )$
], a: [D.])
...
#question([
  5. 设函数 $z = x^(2) + sin y, x = cos t, y = t^(3)$ ，则 $(( (d)z)/( (d)t)) =$ \_\_\_\_\_\_\_\_.
], a: [$- sin 2 t + 3 t^2 cos t^3$])

#question([
  6. 设函数 $z = f(x + y, x - y)$ ，其中 $f$ 可微，则 $(d)z =$
], a: [$(f_1' + f_2') (d)x + (f_1' - f_2') (d)y .$])

#question([
  7. 设函数 $z = f(x, ((x)/(y)))$ ，其中 $f$ 具有二阶连续偏导数，则 $(( partial^2 z)/( partial x partial y)) =$ \_\_\_\_\_\_\_\_.
], a: [$-((1)/(y^2)) f_2' - ((x)/(y^2)) f_(12)'' - ((x)/(y^3)) f_(22)''.$])
...
#question([
  9. 设函数 $z = (x - y)$ ，而 $x = 3 t, y = 4 t^3$ ，求 $(( (d)z)/( (d)t))$ .
], a: [$(3(1 - 4 t^2))( sqrt (1 - (3 t - 4 t^3)^2))$ .])

#question([
  10. 设函数 $z = x y$ ，而 $y = (e)^x$ ，求 $(( (d)z)/( (d)x))$
...
#question([
  1. 已知 $x + y - z = (e)^(x), x (e)^(x) = tan t, y = cos t$ ，则 $.(( (d)z)/( (d)t))|_(t=0) = ( " " )$
A. $((1)/(2))$ 
...
#question([
  3. 设函数 $y = y(x)$ 由方程 $1 + x^(2)y = (e)^(y)$ 所确定，则 $(( (d y))/( (d x))) =$
], a: [$((2 x y)/( (e)^y - x^2))$])

#question([
  4. 设函数 $z = z(x, y)$ 由方程 $x y^2 z = x + y + z$ 所确定，则 $(( partial z)/( partial y)) =$ \_\_\_\_\_\_\_\_.
], a: [$((2 x y z - 1)/(1 - x y^2))$])

#question([
  5. 由方程 $x y z + sqrt (x^2 + y^2 + z^2) = sqrt (2)$ 所确定的函数 $z = z(x, y)$ 在点 $(1,0,-1)$ 处的全微分 $(d)z =$
], a: [$(d x) - sqrt (2) (d y)$])

#prob-type("三、计算题")

#question([
  6. 设 $ln sqrt (x^2 + y^2) = ((y)/(x))$ , 求 $(( (d)y)/( (d)x))$ .
], a: [$((x + y)/(x - y))$])
...
#question([
  8. 设 $ ( (a r r a y)(l) z = x^2 + y^2, \\ x^2 + 2 y^2 + 3 z^2 = 20, (a r r a y) . $ 求 $(( (d y))/( (d x))), (( (d z))/( (d x)))$ .
], a: [$(( (d y))/( (d x))) = -((x(6 z + 1))/(2 y(3 z + 1))),(( (d z))/( (d x))) = ((x)/(3 z + 1)).$])

#question([
  9. 设 $ ( (a r r a y)(l) u = f(u x, v + y), \\ v = g(u - x, v^(2)y), (a r r a y) . $ 其中 $f, g$ 具有一阶连续偏导数，求 $(( partial u)/( partial x)), (( partial v)/( partial x))$ .
], a: [$ (( partial u)/( partial x)) = (-u f_(1)^( )(2 y v g_(2)^( ) - 1) - f_(2)^( ) g_(1)^( ))((x f_(1)^( ) - 1)(2 y v g_(2)^( ) - 1) - f_(2)^( ) g_(1)^( )), (( partial v)/( partial x)) = (g_(1)^( )(x f_(1)^( ) + u f_(1)^( ) - 1))((x f_(1)^( ) - 1)(2 y v g_(2)^( ) - 1) - f_(2)^( ) g_(1)^( )). $])
...
#question([
  4. 曲面 $x^(2)y z - x y^(2)z^(3) = 6$ 在点(3,2，1)处的法线方程为（ ）.
...
#question([
  10. 求曲线 $ ( (a r r a y)(l) x^(2) + y^(2) + z^(2) - 3 x = 0, \\ 2 x - 3 y + 5 z - 4 = 0 (a r r a y) . $ 在点(1,1,1)处的切线及法平面方程
], a: [切线方程为 $((x - 1)/(16)) = ((y - 1)/(9)) = ((z - 1)/(-1))$ , 法平面方程为 $1 6 x + 9 y - z - 24 = 0$ .])

#question([
  11. 求曲面 $(e)^z - z + x y = 3$ 在点(2,1,0)处的切平面及法线方程
], a: [切平面方程为 $x + 2 y - 4 = 0$ ，法线方程为 $ ( (a r r a y)(l) x - 2 = ((y - 1)/(2)), \\ z = $])
...
#question([
  2. 设函数 $z = f(x, y)$ 的全微分 $(d)z = x (d)x + y (d)y$ ，则点(0，0)( ).
A. 不是 $f(x, y)$ 的连续点
...
#question([
  4. 设函数 $f(x, y) = 2 x^(2) + a x + x y^(2) + 2 y$ 在点 $(1, -1)$ 处取得极值，则常数 $a =$ \_\_\_\_\_\_\_\_，极值的类型为 \_\_\_\_\_\_\_\_。
], a: [-5, 极小值.])

#question([
  5. 若要求函数 $u = sin x sin y sin z$ 满足 $x + y + z = (( pi )/(2))$ 且 $x > 0, y > 0, z > 0$ 的条件极值，可构造函数 \_\_\_\_\_\_\_，求得其驻点为 \_\_\_\_\_\_\_，在该点处函数取得 \_\_\_\_\_\_\_。
...
B. $Delta z = f_(x)^( )(x_(0),y_(0)) Delta x + f_(y)^( )(x_(0),y_(0)) Delta y$ 

C. $Delta z = f_(x)^( )(x_(0),y_(0)) (d)x + f_(y)^( )(x_(0),y_(0)) (d)y$ 

D. $Delta z = (d)z + o( rho )$ , 其中 $rho = sqrt (( Delta x)^2 + ( Delta y)^2)$
], a: [D.])
...
#question([
  7. 设函数 $z = x^(y + 1) (x > 0, x != 1)$ ，则 $(d)z =$ \_\_\_\_\_\_\_\_.
], a: [$(y + 1)x^(y) (d)x + x^(y + 1) ln x (d)y .$])
...
#question([
  10. 函数 $z = x y$ 在闭区域 $D = ((x, y) | x >= 0, y >= 0, x + y <= 1)$ 上的最大值为 \_\_\_\_\_\_\_.
], a: [$((1)/(4))$])
...
#question([
  14. 求坐标原点到曲线 $ ( (a r r a y)(l) z = x^2 + y^2, \\ x + y + z = 1 (a r r a y) . $ 的最短和最长距离.
